...
;(function($){
	$.fn.jcDate = function(options) {
		...
		return this.each(function() {
			if($("#jcDate").length == 0){
				$("body").prepend("<div id='jcDate'><input type='hidden' value='' id='dateHideText' /><div id='jcDateTt'><a id='d_prev'></a><div><span></span><b>年</b><samp></samp><b>月</b></div><a id='d_next'></a></div><dl id='jcYeas'></dl><dl id='jcMonth'></dl><div id='jcDayWrap'><dl id='jcDayCon'></dl><div id='jcDateMax'><ul></ul><ul></ul><ul></ul><ul></ul><ul></ul><ul></ul><ul></ul><ul></ul><ul></ul><ul></ul><ul></ul><ul></ul></div><div id='jcDateBtn'><samp>今天</samp><span>清空</span><a class='d_sub' id='d_sub'>确定</a><div id='jcTimeWrap'><input type='text' value='' /><label>:</label><input type='text' value='' /></div></div></div></div>");
			};
			...
			function shFade(show,hide,hide2,bool,index){
				...
				if(bool){
					...
					$b.eq(index).hide();
					$(".alert").hide();//wanghao 修改   2017.3.29
				} else {
					$("#d_prev,#d_next").fadeIn(fadeSpeed);
					$b.show();
					$year.show();
					$month.show();
					$(".alert").show();//wanghao 修改   2017.3.29
				};
			};
