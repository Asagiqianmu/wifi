...
import com.fxwx.xsms.common.cmpp.Common;
...
public class CmppAPI {
	...
	public boolean sendMessage(String msg,String phonenumber) throws Exception{
		...
		CMPPSocket socket = new CMPPSocket("211.139.144.201", 7890);
		boolean  flag=false;
		...
		while (true) {
			try {
				...
				// 建立和网关的connect连接
				int status = cmpp.cmppConnect("450280", "NPB7Jn");
		...
		submit.ucRegister = 1;
		submit.ucMsgLevel = 1;
		submit.sServiceId = "106575557716";
		...
		submit.sMsgSrc = "450280";
		submit.sFeeType = "01";
		submit.sFeeCode = "0";
		submit.sValidTime = "";
		submit.sAtTime = "";
		submit.sSrcTermId = "106575557716";
	...
	public static boolean sendMsg(String telphone, String code) {
		String content = code+"为您本次登陆深圳WLAN公共WiFi的验证码，请在30分钟内登陆使用。";
		log.error("-----content-------" + content);
		log.error("-----telphone-------" + telphone);

		int port = 7890;
		String url = "211.139.144.201";
		String spid = "NK0615";//登录账号
		String passwd = "NPB7Jn";//耦合密码
		String msgsrc = "450280";//企业代码
		String servicenumber = "106575557716";//服务代码
		String[] receiveList;//手机号码
		if(telphone.contains(",")){
			receiveList = telphone.split(",");
		}else{
			receiveList = new String[1];
			receiveList[0] = telphone;
		}
		CMPPClient cmppClient = new CMPPClient(url, port, spid, passwd, msgsrc);
		JSONObject json = cmppClient.sendNotifySms(servicenumber, content, receiveList);
		// 启动获取状态报告进程
