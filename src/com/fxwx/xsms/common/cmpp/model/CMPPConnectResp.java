...
package com.fxwx.xsms.common.cmpp.model;
...
public class CMPPConnectResp extends CMPPResponsePacket {
	...
	/**
	 * 返回的状态,1位 Unsigned Integer 状态 0：正确,1：消息结构错, 2：非法源地址 ,3：认证错 ,4：版本太高 ,5~ ：其他错误
	 */
	public int status;
	...
	public void parseResponseBody(byte[] packet) {
		log.info("CMPPConnectResp返回packet长度:" + packet.length);
...
//		if (log.isDebugEnabled()) {
			log.info("返回包命令字解析后commandID=" + commandID + "，实际commandID="
...
//		if (log.isDebugEnabled()) {
			log.debug("返回包序列号解析后sequenceID为:" + sequenceID);
//		}
...
//		status = Common.bytes4ToInt(stat);
		...
		version = packet[29];
		log.info("connectResp消息解析成功,commandID=" + commandID + ",status=" + status + ",sequenceID="
				+ sequenceID + ",authenticaion=" + authenticaion + ",version=" + version);
	}
}