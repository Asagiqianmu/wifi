...
public class CMPPConnect implements CMPPRequestBody {
	...
	public byte[] getRequestBody() {
		...
		log.info("getRequestBody----------------spid="+spid);
		int cur = 0;
...
//		byte[] authen = getAuthenticatorSource(spid, password, timestamp);
		System.arraycopy(authen, 0, body, cur, authen.length);
		cur += authen.length;

		log.info("getRequestBody----------------version="+version);
		body[cur] = (byte) version;
		cur += 1;
		log.info("getRequestBody----------------timestamp="+timestamp);
	...
	private byte[] md5(String spid) {
		log.info("md5----------------spid="+spid);
		byte sp[] = spid.getBytes();
		byte bzero[] = new byte[9];
		log.info("md5----------------password="+password);
		byte[] bSPpassword = password.getBytes();

		log.info("md5----------------timestamp="+timestamp);
...
//			if (log.isDebugEnabled()) {
				log.info("md5散列码：" + Common.bytes2hex(result));
//			}
		} catch (NoSuchAlgorithmException e) {
