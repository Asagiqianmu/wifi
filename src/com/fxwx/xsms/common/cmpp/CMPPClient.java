...
import com.fxwx.xsms.common.cmpp.model.CMPPConnectStatus;
import com.fxwx.xsms.common.cmpp.model.CMPPSubmitResp;
import com.fxwx.xsms.common.cmpp.model.CMPPSubmitStatus;
import com.fxwx.xsms.common.cmpp.model.SubmitBody;
...
public class CMPPClient {
	 ...
	 * 
	 */
	public JSONObject sendNotifySms(String servicenumber, String msgContent,
			String[] receiveList) {
		...
		try {
			...
			if (connStatus == 0) {
					...
					if (msgContent.getBytes("ISO-10646-UCS-2").length > 140) {
						...
						for (int i = 0; i < messageList.size(); i++) {
							...
							int submitStatus = cmppService.cmppSubmit(
									submitBody, sbresp);
							if (submitStatus != 0) {
								log.error(CMPPSubmitStatus.getDesByCode(submitStatus));
								json.put("result", submitStatus);
								json.put("msg", CMPPSubmitStatus.getDesByCode(submitStatus));
								return json;
							}else{
								log.error("A发送submit消息,发送完毕后等待获得submitresp,短信发送正确。submitStatus="+submitStatus);
								json.put("result", submitStatus);
								json.put("msg", "短信发送成功");
								return json;
							}
						...
						if (submitStatus != 0) {
							...
							log.error("B发送submit消息,发送完毕后等待获得submitresp,短信发送正确。submitStatus="+submitStatus);
							json.put("result", submitStatus);
							json.put("msg", "短信发送成功");
							return json;
						}
				...
				log.error("消息异常："+CMPPConnectStatus.getDesByCode(connStatus));
				json.put("result", connStatus);
				json.put("msg", CMPPConnectStatus.getDesByCode(connStatus));
				return json;
			}
		} catch (IOException e) {
			e.printStackTrace();
	...
	public static SubmitBody getSubmit(String spid, String srcid,
		...
		msgSubmit.setsServiceId("MGD0019901");// 业务类型，是数字、字母和符号的组合
