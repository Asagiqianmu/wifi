...
public class MemcachedUtils {
	...
	static {
		if (flag) {
			try {
				...
				cache = new MemcachedClient(new ConnectionFactoryBuilder().setProtocol(Protocol.BINARY).setAuthDescriptor(ad).build(), AddrUtil.getAddresses(host + ":" + port));
				log.error("初始化========" + cache);
				// cache = new MemcachedClient(new InetSocketAddress(host,
				// 11211));
			} catch (IOException e) {
	...
	/**
	 * 向缓存添加键值对并为该键值对设定过期时间（即多长时间后该键值对从Memcached内存缓存中删除，比如： new
